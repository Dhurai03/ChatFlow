...
- **User Authentication** — Signup, login, persistent session via JWT
- **User Search** — Search other users by name or email in the sidebar
- **One-to-One Conversations** — Start or continue conversations with any user
...
- **Message Status** — Sent ✓ → Delivered ✓✓ → Read ✓✓ (highlighted)
...
**Frontend:** React 19, Vite, React Router v7, Axios, Socket.IO Client, Vanilla CSS
...
├── client/
│   └── src/
│       ├── components/     # ChatLayout, Sidebar, ChatWindow, MessageBubble, etc.
│       ├── context/        # AuthContext, SocketContext
│       ├── hooks/          # useConversations, useMessages
│       ├── pages/          # LoginPage, SignupPage, ChatPage
│       ├── services/       # api.js, conversationService, messageService, userService
...
- On login/signup, the server returns a **JWT** signed with `JWT_SECRET`, expiring in 7 days.
...
| Field           | Type       | Notes                            |
|-----------------|------------|----------------------------------|
| `participants`  | [ObjectId] | Refs to two User documents       |
...
6. User B opens conversation → emits `messages:read` → sender sees ✓✓ turn blue
...
| PATCH  | /api/messages/:messageId/status                  | Yes  | Update a single message status     |
...
| Server → Client | `messages:read`     | `{ conversationId }`                              |
